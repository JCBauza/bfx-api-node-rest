...
import MockSrv from 'bfx-api-mock-srv'
const { MockRESTv2Server } = MockSrv
import Models from 'bfx-api-node-models'
const { Order } = Models

const getTestREST2 = (args: Record<string, unknown> = {}) => {
  return new RESTv2({
    apiKey: 'dummy',
    apiSecret: 'dummy',
...
const getTestFundingOffer = (): unknown[] => ([
...
const getTestFundingLoan = (): unknown[] => ([
...
const getTestFundingCredit = (): unknown[] => ([
...
const auditTestFundingOffer = (fo: Record<string, unknown> = {}) => {
...
const auditTestFundingLoan = (fl: Record<string, unknown> = {}) => {
...
const auditTestFundingCredit = (fc: Record<string, unknown> = {}) => {
...
describe('RESTv2 integration (mock server) tests', () => {
  let srv: MockRESTv2Server | null = null
  ...
  methods.forEach((m) => {
    ...
    it(`${name}: fetches expected data`, (done) => {
      ...
      args.push((err: Error | null, res: unknown) => {
        if (err) {
          return done(err)
        }
      ...
      ;(r as Record<string, (...a: unknown[]) => unknown>)[name].apply(r, args)
    })
  })
  ...
  it('correctly parses funding offer response', async () => {
    ...
    const [fo] = await r.fundingOffers({ symbol: 'fUSD' }) as Record<string, unknown>[]
  ...
  it('correctly parses funding loan response', async () => {
    ...
    const [fo] = await r.fundingLoans({ symbol: 'fUSD' }) as Record<string, unknown>[]
  ...
  it('correctly parses funding credit response', async () => {
    ...
    const [fc] = await r.fundingCredits({ symbol: 'fUSD' }) as Record<string, unknown>[]
  ...
  it('correctly parses accountSummary response', async () => {
    ...
    const summary = await r.accountSummary() as Record<string, unknown>
  ...
  it('correctly parses symbolDetails response', async () => {
    ...
    const details = await r.symbolDetails() as Record<string, unknown>[]
  ...
  it('correctly parses keyPermissions response', async () => {
    ...
    const perms = await r.keyPermissions() as Record<string, unknown>[]
  ...
  it('correctly executes closePosition method', async () => {
    ...
    const order = await r.closePosition({ position_id: 145287699 }) as InstanceType<typeof Order>
