...
describe('rest integration test', () => {
  ...
  it('should get the fundingbook asks, zero bids, 100 asks', (done) => {
    ...
    server.listen(PORT, () => {
      bhttp.fundingbook('USD', opts, (err: Error | null, data: Record<string, unknown[]>) => {
        if (err) throw err
        assert.ok(data)
  ...
  it('new_order -- post_only: postonly used and true', (done) => {
    ...
    server = http.createServer((req: http.IncomingMessage, res: http.ServerResponse) => {
      ...
      const payload = JSON.parse(
        Buffer.from(req.headers['x-bfx-payload'] as string, 'base64').toString('ascii')
      )
    ...
    server.listen(PORT, () => {
      bhttp.new_order('btcusd', '0.1', '0.1', 'bitfinex', 'buy', 'exchange limit', false, true, (err: Error | null, _data: unknown) => {
        if (err) throw err
        done()
      })
    })
  ...
  it('new_order -- post_only: postonly not used and hidden true', (done) => {
    ...
    server.listen(PORT, () => {
      bhttp.new_order('btcusd', '0.1', '0.1', 'bitfinex', 'buy', 'exchange limit', true, (err: Error | null, _data: unknown) => {
        if (err) throw err
        done()
      })
    })
  ...
  it('new_order -- post_only: postonly not used and hidden not used', (done) => {
    ...
    server.listen(PORT, () => {
      bhttp.new_order('btcusd', '0.1', '0.1', 'bitfinex', 'buy', 'exchange limit', (err: Error | null, _data: unknown) => {
        if (err) throw err
        done()
      })
    })
