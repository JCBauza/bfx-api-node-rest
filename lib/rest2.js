...
class RESTv2 {
   ...
   * @param {string} params.wallet             - wallet i.e exchange, margin
   * @param {string} params.method             - protocol method i.e bitcoin, tetherus
   * @param {number} [params.amount]           - amount of withdrawal (not used for LNX)
   * @param {string} [params.address]          - destination address (not used for LNX)
   * @param {string} [params.invoice]          - used instead of address for LNBTC (method: 'lnx')
   * @param {string} [params.payment_id]       - specify a tag/memo/etc
   * @param {number} [params.fee_deduct]       - set to 1 to deduct the withdrawal fee from the withdrawal amount
   * @param {string} [params.note]             - optional note for withdrawal
   * @param {boolean} [params.travel_rule_tos] - flag to voluntarily send travel rule details for withdrawal
   * @param {string} [params.vasp_did]         - virtual asset provider identifier (see /v2/int/vasps), optional info
   *                                             for travel rule purpose, if missing on /v2/int/vasps or self custody
   *                                             ignore the field
   * @param {string} [params.vasp_name]        - virtual asset provider name (see /v2/int/vasps), optional info for
   *                                             travel rule purpose, if missing on /v2/int/vasps provide manual input,
   *                                             if self custody ignore the field
   * @param {string} [params.dest_firstname]   - destination entity first name for travel rule purpose
   * @param {string} [params.dest_lastname]    - destination entity last name for travel rule purpose
   * @param {string} [params.dest_corp_name]   - destination entity corporate name for travel rule purpose, use either
   *                                             dest_firstname + dest_lastname or dest_corp_name
   * @param {Function} [cb]                    - legacy callback
   * @returns {Promise} p
   */
   ...
   *
   * @param {object} params           - parameters
   * @param {object} params.order     - new algo order
   * @param {string} order.alias      - alias or name for the order
   * @param {string} order._symbol    - trading pair symbol
   * @param {string} order.currency   - base or quote currency
   * @param {'buy'|'sell'} order.action  - buy or sell side
   * @param {number} order.amount      - amount in base or quote currency
   * @param {'daily'|'weekly'|'monthly'} order.recurrence  - order frequency
   * @param {Date} [order.startedAt]     - start date of orders execution
   * @param {Date} [order.endedAt]   - algo order will expire on this date (null if endless is true)
   * @param {boolean} [order.endless] - order never expires if set to true (null if endedAt date is set)
   * @param {Function} [cb] - legacy callback
   * @returns {Promise} p
   */
  submitRecurringAlgoOrder (params = {}, cb = null) {
    const { order } = params
   ...
   *
   * @param {object} params              - parameters
   * @param {string} params.algoOrderId  - algo order id to get
   * @param {Function} [cb] - legacy callback
   * @returns {Promise} p
   */
  getRecurringAlgoOrder (params = {}, cb = null) {
   ...
   *
   * @param {object} params             - parameters
   * @param {object} params.order       - algo order to update
   * @param {string} order.algoOrderId  - algo order id
   * @param {string} [order.alias]      - alias or name for the order
   * @param {string} [order._symbol]    - trading pair symbol
   * @param {string} [order.currency]   - base or quote currency
   * @param {'buy'|'sell'} [order.action]  - buy or sell side
   ...
   * @returns {Promise} p
   */
  updateRecurringAlgoOrder (params = {}, cb = null) {
    const { order } = params
   ...
   *
   * @param {object} params               - parameters
   * @param {string} params.algoOrderId   - algo order id to cancel
   * @param {Function} [cb] - legacy callback
   * @returns {Promise} p
   */
  cancelRecurringAlgoOrder (params = {}, cb = null) {
    const { algoOrderId } = params
   ...
   * @param {string} [params.filter.alias]    - search filter for alias field
   * @param {Date} [params.filter.startedAt]  - search filter for startedAt field
   * @param {Date} [params.filter.endedAt]    - search filter for endedAt field
   * @param {boolean} [params.filter.endless]  - search filter for endless field
   * @param {'daily'|'weekly'|'monthly'} [params.filter.recurrence]  - search filter for recurrence field
   * @param {'buy'|'sell'} [params.filter.action]  - search filter for action field
   * @param {string} [params.filter._symbol]       - search filter for _symbol field
   * @param {string} [params.filter.currency]      - search filter for currency field
   * @param {Function} [cb] - legacy callback
   * @returns {Promise} p
   */
